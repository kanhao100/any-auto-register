...
def relogin_chatgpt_account(
    ...
    *,
    ...
    if not email:
        raise RuntimeError("账号缺少邮箱，无法二次登录")
    if not password:
        raise RuntimeError("账号缺少密码，无法二次登录")
    ...
    if callable(log_fn):
        ...
        if mailbox_info.get("message"):
            log_fn(f"[relogin] 邮箱说明: {mailbox_info['message']}")
    if mailbox_service is not None:
        mailbox_service.prime()
        if callable(log_fn):
            log_fn("[relogin] 已建立邮箱基线，后续可轮询 OTP")
    ...
    for attempt in attempts:
        ...
        tokens = oauth_client.login_and_get_tokens(
            ...
            login_source=f"manual_relogin:{attempt['label']}",
        )
        if tokens:
        ...
        if callable(log_fn):
            log_fn(
                f"[relogin] mode={attempt['label']} 失败: {last_error or '未返回 tokens'}"
            )

    if not tokens:
        if not mailbox_info.get("available") and mailbox_info.get("message"):
            detail = f"{last_error or '二次登录失败'}；{mailbox_info['message']}"
        else:
            detail = last_error or "二次登录失败"
